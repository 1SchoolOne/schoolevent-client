...
const Calendar = () => {
	...
	const handleYearSelectChange = useCallback((newYear: string) => {
		const updatedValue = currentDate.clone().year(Number(newYear));
		setCurrentDate(updatedValue);
	}, [currentDate, setCurrentDate]);

	const handleMonthSelectChange = useCallback((newMonth: string) => {
	...
	const headerRender = useCallback(({ value }: HeaderRenderParams) => {
		const start = 0;
		const end = 12;
		const monthOptions = [];
	...
	}, [handlePrevMonth, handleNextMonth, handleYearSelectChange, handleMonthSelectChange, currentDate, years]);
